package com.masget.oauth.core.controller;
...
//			String username = (String)subject.getPrincipal();
			...
			if (responseType.equals(ResponseType.CODE.toString())) {
				...
				authorizeCode.setClientId( oauthRequest.getClientId() );
				authorizeCode.setCode( authorizationCode );
				authorizeCode.setUserName( username );
				authorizeCode.setResponseType( responseType );
				authorizeCode.setScope( oauthRequest.getScopes() );
			...
			// 返回错误消息（如?error=）
			final OAuthResponse errResponse = OAuthASResponse.errorResponse(HttpServletResponse.SC_FOUND).error(e).location(redirectUri).buildQueryMessage();
			HttpHeaders headers = new HttpHeaders();
			headers.setLocation(new URI(errResponse.getLocationUri()));
			return new ResponseEntity(headers, HttpStatus.valueOf(errResponse.getResponseStatus()));
		}
	}
